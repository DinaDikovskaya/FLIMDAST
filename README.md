...
In the Single Object Time Course tab, select the condition, time point and the reference status of first measurement, and add locations of the files describing it by clicking the appropriate “Choose file” buttons and navigating to the requested files. If the files are in the same folder, switching on the “autofill file” after the fluorescence lifetime file has been located will automatically locate the other files for the same measurement (e.g. the photon number files for the entire cell and, if exist, for selected regions) within that folder, following the user-defined name replacement rule.
